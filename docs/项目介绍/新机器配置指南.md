...
### 2.5 （crypto-analysis 用）SageMath

仅当使用 `crypto-analysis` Agent（RSA/格/ECC 等密码学题）时需要。**装进上面同一个 venv**，保持单一虚拟环境架构（**不要用 conda 另起环境**）：

```bash
~/bw-security-analysis/.venv/bin/pip install sagemath-standard
```

- 体积大（数 GB）、安装慢（可能十几分钟、部分需编译），属**一次性安装**。
- macOS 若 pip 路线卡住，退路：`brew install --cask sage`（系统级，`detect_env.py` 也能检测到）。
- 验证：`~/bw-security-analysis/.venv/bin/sage --version` 或 `~/bw-security-analysis/.venv/bin/python -c "import sage"`。
- `detect_env.py` 会自动检测；缺失时 crypto-analysis 对格/Coppersmith/ECC 攻击会提示先装 SageMath，简单 RSA/古典/对称题可用 gmpy2+sympy 替代。
